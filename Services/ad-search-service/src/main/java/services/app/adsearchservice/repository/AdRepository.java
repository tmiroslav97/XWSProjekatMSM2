...
public interface AdRepository extends JpaRepository<Ad, Long> {
    ...
    @Query("SELECT ad FROM Ad ad, CarCalendarTerm calendar WHERE " +
            "calendar.startDate<=(?3) AND calendar.endDate>=(?4) AND ad.id = calendar.ad.id AND " +
            "ad.deleted=(?1) AND ad.location=(?2) AND ad.car.carManufacturer=(?5) AND " +
            "ad.car.carModel=(?6) AND ad.car.carType=(?7) AND ad.car.mileage<=(?8)  AND " +
            "((ad.car.distanceLimitFlag='LIMITED' AND ad.car.distanceLimit>=(?9)) OR ad.car.distanceLimitFlag='UNLIMITED') " +
            " AND ad.car.gearboxType=(?10) AND ad.car.fuelType=(?11) AND ad.car.childrenSeatNum=(?12)" +
            " AND ad.car.cdw=(?13) AND " +
            "ad.pricePerDay>=(?14) AND ad.pricePerDay<=(?15)" +
            "")
